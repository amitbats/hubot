...
jenkinsLast = (msg) ->
    url = process.env.HUBOT_STASH_URL
    job = msg.match[1]
...
module.exports = (robot) ->
  ...
  robot.respond /jenkins help/i, (msg) ->
    msg.send "jenkins b <jobNumber> - builds the job specified by jobNumber. List jobs to get number"
    msg.send "jenkins build <job> - builds the specified Jenkins job"
    msg.send "jenkins build <job>, <params> - builds the specified Jenkins job with parameters as key=value&key2=value2"
    msg.send "jenkins list <filter> - lists Jenkins jobs"
    msg.send "jenkins describe <job> - Describes the specified Jenkins job"
    msg.send "jenkins last <job> - Details about the last build for the specified Jenkins job"
